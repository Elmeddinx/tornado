    ...
    if (dropdownMenu) {
        dropdownMenu.addEventListener('click', function (e) {
            if (e.target.closest('.dropdown-item')) {
                e.preventDefault();
                const selectedLang = e.target.closest('.dropdown-item').getAttribute('data-lang');
                ...
                const updatedDropdownMenu = Object.keys(languages).filter(lang => lang !== selectedLang).map(lang => {
                    return `
                        <li>
